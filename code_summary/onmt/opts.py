...
def model_opts(parser):
    ...
    group.add('--dec_layers', '-dec_layers', type=int, default=1,
    ...
    group.add('--enc_rnn_size', '-enc_rnn_size', type=int, default=300,
              help="Size of encoder rnn hidden states. "
                   "Must be equal to dec_rnn_size except for "
                   "speech-to-text.")
    group.add('--dec_rnn_size', '-dec_rnn_size', type=int, default=300,
...
def train_opts(parser):
    ...
    group.add('--valid_batch_size', '-valid_batch_size', type=int, default=64,
    ...
    group.add('--warmup_steps', '-warmup_steps', type=int, default=2000,
...
def translate_opts(parser):
    ...
    group.add('--model', '-model', dest='models', metavar='MODEL',
            nargs='+', type=str, default=['../code_summary/demo-model_step_100000.pt'],
              help="Path to model .pt file(s). "
                   "Multiple models can be specified, "
                   "for ensemble decoding.")
    ...
    group.add('--review', '-review', type=str, required=False,
              help="Source sequence to decode")

    # group.add('--src', '-src', required=True,
    #           help="Source sequence to decode (one line per "
    #                "sequence)")
    group.add('--src', '-src', default='../code_summary/data/src-test.txt',
              help="Source sequence to decode (one line per "
                   "sequence)")
    group.add('--src_dir', '-src_dir', default="",
              help='Source directory for image or audio files')
    ...
    # group.add('--replace_unk', '-replace_unk', default=True, action="store_true",
    ...
    group.add('--replace_unk', '-replace_unk', default=True,
              help="Replace the generated UNK tokens with the "
                   "source token that had highest attention weight. If "
                   "phrase_table is provided, it will look up the "
                   "identified source token and give the corresponding "
    ...
    group.add('--verbose', '-verbose', default=False,
              help='Print scores and predictions for each sentence')
    group.add('--log_file', '-log_file', type=str, default="",
              help="Output logs to a file under this path.")
    group.add('--log_file_level', '-log_file_level', type=str,
