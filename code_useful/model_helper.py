...
from my_models import Useful_predict
...
class MyDataset(torch.utils.data.Dataset):
    def __init__(self, fname, tokenizer, max_seq_len, max_sent_len):
        ...
        for i in range(len(df)):
            ...
            denominator = row['HelpfulnessDenominator']
            if denominator > 0:
                text_raw = row['Text']
                sent_raw = split_sentence(text_raw)

                if len(sent_raw) < max_sent_len:
                    sent_num = [1] * len(sent_raw) + [0] * (max_sent_len - len(sent_raw))
                    sent_num = np.array(sent_num)

                    sent_raw[0] = '[CLS] ' + sent_raw[0]
                    sent_raw[-1] = sent_raw[-1] + ' [SEP]'
                    sent_raw_indices, masks = tokenizer.sent_to_sequence(sent_raw)
                    sent_masks = []
                    for item in masks:
                    ...
                    data = {
                        'text_raw_indices': sent_raw_indices,
                        'sent_num': sent_num,
                        'sent_masks': sent_masks,
                        'useful_polarity': useful_polarity,
                    }
    ...
    def __init__(self, opt):
        ...
        if self.opt.if_train:
            ...
            self.train_dataset = MyDataset(
                self.opt.train_data_file, self.tokenizer,
                self.opt.max_seq_len, self.opt.max_sent_len
            )
        ...
        self.test_dataset = MyDataset(
            self.opt.test_data_file, self.tokenizer,
            self.opt.max_seq_len, self.opt.max_sent_len
        )
        ...
        # Step3: Model
        if self.opt.model == "useful-predict":
            base_model = BertModel.from_pretrained(self.opt.pretrained_bert_name)
            self.model = Useful_predict(
                base_model,
                self.opt.dropout,
                self.opt.bert_dim,
                self.opt.polarities_dim,
            ).double().to(self.opt.device)
            self.cols = ['text_raw_indices', 'sent_masks', 'sent_num']
        self._reset_params()
    ...
    def train(self):
        ...
        for epoch in range(1, self.opt.num_epoch + 1):
            ...
            for i_batch, sample_batched in enumerate(train_dataloader):
                ...
                self.model.train()
                inputs = [
                    sample_batched[col].to(self.opt.device) for col in self.cols
                ]
                targets = sample_batched['useful_polarity'].to(self.opt.device)
    ...
    def test(self):
        ...
        with torch.no_grad():
            for t_batch, t_sample_batched in enumerate(test_dataloader):
                ...
                t_targets = t_sample_batched['useful_polarity'].to(self.opt.device)
